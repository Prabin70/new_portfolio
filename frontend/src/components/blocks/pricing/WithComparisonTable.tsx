...
const sections: {
  // Add explicit type definition for sections
  name: string;
  features: {
    name: string;
    tiers: { [key: string]: boolean | string }; // <-- FIX: Add index signature here
  }[];
}[] = [
  {
    name: "Features",
                  ...
                  <li key={section.name}>
                    <ul role="list" className="space-y-4">
                      {section.features.map((feature) =>
                        feature.tiers[tier.name] ? ( // Accessing with tier.name is now valid
